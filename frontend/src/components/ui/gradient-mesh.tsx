...
interface NovatrixProps {
  ...
  waveSpeed?: number;
  grain?: number; // NEW
  className?: string; // added so we can pass opacity etc.
}
...
export function GradientMesh(props: NovatrixProps) {
  ...
  useEffect(() => {
    ...
    const renderer = new Renderer({ alpha: true }); // Make background transparent if needed
    const gl = renderer.gl;
    // transparent clear color so it blends with white background
